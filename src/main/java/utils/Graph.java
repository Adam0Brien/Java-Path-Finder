...
//We're not at the goal node yet, so...
    ...
    //Interface method to allow just the starting node and the goal node data to match to be specified
    public static <T> List<Pixel> findBreadthFirstPathInterface(Pixel startPixel, T lookingfor, Image image) {
        List<List<Pixel>> agenda = new ArrayList<>(); //Agenda comprised of path lists here!
        List<Pixel> firstAgendaPath = new ArrayList<>(), resultPath;
        firstAgendaPath.add(startPixel);
        agenda.add(firstAgendaPath);
        resultPath = findBreadthFirstPath(agenda, null, lookingfor, image); //Get single BFS path (will be shortest)
        Collections.reverse(resultPath); //Reverse path (currently has the goal node as the first item)
        return resultPath;
    }

    //Agenda list based breadth-first graph search returning a single reversed path (tail recursive)
    public static <T> List<Pixel> findBreadthFirstPath(List<List<Pixel>> agenda,
                                                       List<Pixel> encountered, T lookingfor, Image image) {
        if (agenda.isEmpty()) return null; //Search failed
        List<Pixel> nextPath = agenda.remove(0); //Get first item (next path to consider) off agenda
        Pixel currentPixel = nextPath.get(0); //The first item in the next path is the current node
        if (currentPixel.equals(lookingfor))
            return nextPath; //If that's the goal, we've found our path (so return it)
        if (encountered == null)
            encountered = new ArrayList<>(); //First node considered in search so create new (empty)
        if (!encountered.contains(currentPixel)) {
            //encountered list
            encountered.add(currentPixel);//Record current node as encountered so it isn't revisited again
            List<Pixel> children = new ArrayList<>();// create a list of all pixels surrounding the current one
//Pixels above the current one
            children.add(new Pixel(currentPixel.getXCorrd() - 1, currentPixel.getYCoord() - 1)); //top Left
            children.add(new Pixel(currentPixel.getXCorrd(), currentPixel.getYCoord() - 1)); //Top Middle
            children.add(new Pixel(currentPixel.getXCorrd() + 1, currentPixel.getYCoord() - 1)); // Top Right
            //Pixels to the right and left of current one
            children.add(new Pixel(currentPixel.getXCorrd() - 1, currentPixel.getYCoord())); // Left of current
            children.add(new Pixel(currentPixel.getXCorrd() + 1, currentPixel.getYCoord())); // Right of Current
            //Pixels below the current one
            children.add(new Pixel(currentPixel.getXCorrd() - 1, currentPixel.getYCoord() + 1)); //bottom left
            children.add(new Pixel(currentPixel.getXCorrd(), currentPixel.getYCoord() + 1)); // bottom middle
            children.add(new Pixel(currentPixel.getXCorrd() + 1, currentPixel.getYCoord() + 1)); // bottom right
            for (Pixel childPixel : children) {
                if (!encountered.contains(childPixel)) {
                    List<Pixel> newPath = new ArrayList<>(nextPath);

                    newPath.add(0, childPixel);
                    agenda.add(newPath);
                }
            }
        }
        return findBreadthFirstPath(agenda, encountered, lookingfor, image); //Tail call
    }

    private static List<Pixel> createChildern(Pixel currentPixel) {
